...
async function userModify(user,success,fail){
  await api.put(`/userapi/user`, JSON.stringify(user)).then(success).catch(fail);
}

async function delUser(userid, success, fail) {
  await api.delete(`/userapi/user/${userid}`).then(success).catch(fail);
}

export { login, findById, tokenRegeneration, logout, regist, userModify ,delUser};
